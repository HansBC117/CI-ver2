...
L = size(signal,1);
if round(L/2) ~= L/2
	error('The signal must have an even number of elements')
end
...
%	plot(f,abs(KERNEL(1:L/2)),'-b',f,abs(KERNEL2(1:L/2)),'-r')
	...
	xlabel('Frequency [Hz]')
	ylabel('Amplitude [Same units as ingoing signal]')
	xlim([0 f(end)])
